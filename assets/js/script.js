...
function initStyle() {
  ...
  // Function to remove theme styles (for "Auto" option)
  function removeThemeStyles() {
    document.documentElement.style.removeProperty('--b');
    document.documentElement.style.removeProperty('--bs');
  ...
  function applyTheme(theme) {
    if (theme === 'dark') {
      document.documentElement.style.setProperty('--b', 'rgb(31,31,31)');
      document.documentElement.style.setProperty('--bs', '#171717');
      document.documentElement.style.setProperty('--t', 'white');
      document.documentElement.style.setProperty('--ts', '#989898');
      document.documentElement.style.setProperty('--bc', '#171717');
      document.documentElement.style.setProperty('--h', 'rgba(238, 238, 238, 0.5)');
    } else if (theme === 'light') {
      document.documentElement.style.setProperty('--b', 'white');
      document.documentElement.style.setProperty('--bs', '#ebebeb');
      document.documentElement.style.setProperty('--t', 'black');
      document.documentElement.style.setProperty('--ts', '#6b6b6b');
      document.documentElement.style.setProperty('--bc', '#e5e5e5');
      document.documentElement.style.setProperty('--h', 'rgb(134, 134, 134 / 50%)');
    } else if (theme === 'auto') {
      removeThemeStyles(); // Remove any applied theme styles for "Auto"
    }
  }
  ...
  if (savedFont) {
    document.querySelectorAll("*").forEach(el => {
      el.style.setProperty('font-family', savedFont); // Apply to all elements inside body
    });
    if (fontDropdown) {
  ...
  if (fontDropdown) {
    fontDropdown.addEventListener('change', function() {
      ...
      document.querySelectorAll("body *").forEach(el => {
        el.style.setProperty('font-family', selectedFont); // Apply font to all elements inside body
      });
      setCookie('selectedFont', selectedFont, 30); // Save font in cookie for 30 days
    });
  }
  ...
  if (themeDropdown) {
    themeDropdown.value = savedTheme || 'light'; // Set default value based on saved theme

    // Event listener for theme dropdown
    themeDropdown.addEventListener('change', function() {
      const selectedTheme = this.value;
      setCookie('selectedTheme', selectedTheme, 30); // Save selected theme in cookie for 30 days
